// Server-side mirror of vault data so the API can run standalone.
export interface ServerVault {
  id: string; slug: string; creator: string; handle: string;
  category: 'music' | 'film' | 'gaming' | 'creator' | 'sports';
  avatarColor: string; tagline: string; description: string;
  tvl: number; apy: number; apy30d: number; apy90d: number;
  fans: number; capacity: number; boost: number; lockup: number;
  riskScore: number; momentum: number;
...
function makeMonthly(base: number, drift: number) {
  // Deterministic for server (no Math.random per-request)
  return months.map((m, i) => ({
    m,
    tvl: Math.round(base * (1 + drift * i)),
    revenue: Math.round(base * 0.04 * (1 + drift * i)),
  }));
}

export const VAULTS: ServerVault[] = [
  { id:'v-001', slug:'noor', creator:'NOOR', handle:'@noor', category:'music', avatarColor:'#70F3D8',
    ...
    revenue:[
      {source:'Streaming',usd:142000,share:0.34},
      {source:'Drops & Merch',usd:98000,share:0.24},
      {source:'Live & Touring',usd:121000,share:0.29},
      {source:'IP & Sync',usd:53000,share:0.13},
    ],
  ...
  { id:'v-002', slug:'rin', creator:'RIN ASUKA', handle:'@rin', category:'creator', avatarColor:'#9CFBE4',
    ...
    fans:12104, capacity:20000, boost:1.30, lockup:90, riskScore:4, momentum:0.48,
    revenue:[
      {source:'YouTube AdSense',usd:78000,share:0.18},
      {source:'Streaming Licenses',usd:162000,share:0.38},
      {source:'Merch & Print',usd:84000,share:0.20},
      {source:'IP Licensing',usd:102000,share:0.24},
    ],
  ...
  { id:'v-003', slug:'kova', creator:'KOVA', handle:'@kova', category:'gaming', avatarColor:'#4DE6C0',
    ...
    revenue:[
      {source:'Game Sales',usd:88000,share:0.55},
      {source:'DLC',usd:28000,share:0.18},
      {source:'Tournaments',usd:16000,share:0.10},
      {source:'Merch',usd:27000,share:0.17},
    ],
  ...
  { id:'v-004', slug:'lumen', creator:'LUMEN', handle:'@lumen', category:'music', avatarColor:'#3FE0BC',
    ...
    fans:22500, capacity:30000, boost:1.15, lockup:180, riskScore:2, momentum:0.38,
    revenue:[
      {source:'Production Royalties',usd:220000,share:0.50},
      {source:'Sample Licensing',usd:78000,share:0.18},
      {source:'Beat Marketplace',usd:65000,share:0.15},
      {source:'Mentorship',usd:75000,share:0.17},
    ],
  ...
  { id:'v-005', slug:'vex', creator:'VEX FC', handle:'@vexfc', category:'sports', avatarColor:'#1FB89A',
    ...
    revenue:[
      {source:'Prize Pools',usd:145000,share:0.31},
      {source:'Sponsorships',usd:195000,share:0.42},
      {source:'Content Rev',usd:60000,share:0.13},
      {source:'Merch',usd:65000,share:0.14},
    ],
  ...
  { id:'v-006', slug:'sable', creator:'SABLE & CO.', handle:'@sable', category:'film', avatarColor:'#70F3D8',
    ...
    revenue:[
      {source:'Distribution',usd:92000,share:0.38},
      {source:'Streaming Licenses',usd:78000,share:0.32},
      {source:'Festival Awards',usd:22000,share:0.09},
      {source:'IP Development',usd:50000,share:0.21},
    ],
...
export function findVault(slugOrId: string): ServerVault | undefined {
  return VAULTS.find(v => v.slug === slugOrId || v.id === slugOrId);
}
